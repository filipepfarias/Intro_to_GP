...
\begin{column}{0.45\textwidth}
	...
	\visible<2->{It's reasonable to see that our model start's to differ from the $y$ and starts to interpolate the noise.}

	\visible<3->{Last, a basic concept it's that larger weights could imply in massive system inputs, and this sometimes it's wanted to be avoided.}
\end{column}
...
\begin{frame}{\insertsubsection}
We could observe that taking the derivative with respect to $\mathbf{w}$, our expression becomes closer to the \textit{error function} presented previously, added some dependency of $\beta$

\begin{align*}
	E(\mathbf{w}) \triangleq \frac{1}{2} \sum_{n=1}^N \left\{ y_n -  t_n \right\}^2
\end{align*}

Then some behaviors could be expected, as the \textbf{over-fitting}. But there's a difference between the equations by the term $\beta$.
...
So, here our objective is to find the parameters to minimize the error and maximize the log probability. But it's reasonable to think that we'll face the same \textbf{over-fitting} problem presented before. Added to the fact that we don't apply yet a fully \textit{bayesian} treatment of regression, let's introduce some machinery 
