...
Sometimes, a good first assumption can make the difference to the estimation, and one may want to put its beliefs in the model, before observe some data, i.e. make some \textit{prior} assumptions. This strategy is well defined by the Bayesian statistics and could help if we assume a probabilistic model. For this, we can accept some \textit{uncertainty} over the model or its parameters.
