...
\begin{appendices}
  ...
  Be the linear model $f(x,\mathbf{w}) = \boldsymbol{\phi}(x)^\top \mathbf{w}$. Suppose $\Phi = \left[ \boldsymbol{\phi}(x_1),\dots,\boldsymbol{\phi}(x_N) \right]^\top$, then $\Phi$ will be of the form
