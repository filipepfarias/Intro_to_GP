...
phi = @(a)(2 * [cos(bsxfun(@times,a(:,1)/8,pi.*[0:8])), sin(bsxfun(@times,a(:,2)/8,pi.*[1:8]))]);
mu = zeros(F,1);
...
for f = 1:fr
    clf; hold on;
    plot3(X1,X2,T3,'o','MarkerFaceColor',dre,'MarkerEdgeColor',...
        'none','MarkerSize',2);
    surf(x1,x2,max(min(m3 + 2 * std3,200),-200),'FaceLighting',...
        'gouraud','FaceColor',lightdre,'EdgeColor',dre,'EdgeLighting',...
        'gouraud','FaceAlpha',.3,'EdgeColor','none');
    surf(x1,x2,max(min(m3 - 2 * std3,200),-200),'FaceLighting',...
        'gouraud','FaceColor',lightdre,'EdgeColor',dre,'EdgeLighting',...
        'gouraud','FaceAlpha',.3,'EdgeColor','none');
    ys1 = L' * s1(:,f); ys1 =m3 + reshape(ys1,size(x1));
    surf(x1,x2,ys1,'FaceLighting','gouraud','FaceColor',lightdre,...
        'EdgeColor',dre,'EdgeLighting','gouraud','EdgeAlpha',.5);
    light('Position',[-1 0 400],'Style','infinite'); material dull; view(3);
    zlim([-2,2]);
    drawnow; pause(0.02)
end
